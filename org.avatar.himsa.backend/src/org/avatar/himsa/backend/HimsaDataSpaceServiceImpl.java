...
public class HimsaDataSpaceServiceImpl implements DataSpaceService {
	...
	@Activate
	public HimsaDataSpaceServiceImpl(
		...
		if(policyId != null && getAssetPolicy(policyId) == null) {
			...
			if(assetPolicyInDataSpace == null) {
				LOGGER.severe(String.format("Error creating policy %s in data space", policyId));
			}
		} else {
		...
		if(contractDefinitionInDataSpace == null) {
			LOGGER.warning(String.format("Error creating contract in data space"));
		}
	...
	private void createInitialAssets(String... assetIds) {
		for(String assetId : assetIds) {
			if(getAsset(assetId) == null) {
				...
				createAssetInDataSpace(assetId, "http://himsa:8088/himsa/rest/patient/query/{requestId}", assetType, String.format("HIMSA data in %s format", assetType));
			} else {
